...
import me.offeex.exethirteen.utils.listenForPackageChanges
import me.offeex.exethirteen.utils.signaturesCompat
...
object PluginManager {
    ...
    fun fetchPlugins() = synchronized(this) {
        if (receiver == null) receiver = Core.app.listenForPackageChanges {
            synchronized(this) {
                receiver = null
                cachedPlugins = null
            }
    ...
    private fun initNative(configuration: PluginConfiguration): InitResult? {
        ...
        val providers = Core.app.packageManager.queryIntentContentProviders(
        ...
        try {
            return initNativeFast(Core.app.contentResolver, options, uri)?.let { InitResult(it, options, isV2) }
        } catch (t: Throwable) {
        ...
        try {
            return initNativeSlow(Core.app.contentResolver, options, uri)?.let { InitResult(it, options, isV2) }
        } catch (t: Throwable) {
            failure?.also { t.addSuppressed(it) }
            throw t
        }
    ...
    fun ComponentInfo.loadString(key: String) = when (val value = metaData.get(key)) {
        is String -> value
        is Int -> Core.app.packageManager.getResourcesForApplication(applicationInfo).getString(value)
        null -> null
