...
class ServiceNotification(
    private val service: BaseService.Interface, channel: String,
    ...
    private val callback: IShadowsocksServiceCallback by lazy {
        object : IShadowsocksServiceCallback.Stub() {
            ...
            override fun trafficUpdated(profileId: String, stats: TrafficStats) {
                ...
                builder.apply {
                    service as Context
                    val txTotal = Formatter.formatFileSize(service, stats.txTotal)
                    val rxTotal = Formatter.formatFileSize(service, stats.rxTotal)
                    setSubText("$txTotal↑\t$rxTotal↓")
                }
                show()
            }

            override fun trafficPersisted(profileId: String) {}
        }
    }
